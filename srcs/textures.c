...
/*   Updated: 2019/11/11 18:42:29 by sbrella          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		get_texture(char *name, t_txture *texture, char *path)
{
	...
	an_surf = SDL_CreateRGBSurface(0, TEXTURE_WIDTH,
	TEXTURE_HEIGHT, 32, RMASK, GMASK, BMASK, AMASK);
	SDL_BlitScaled(surf, NULL, an_surf, NULL);
	ft_memdel((void **)&fmt);
	texture->width = an_surf->w;
	texture->height = an_surf->h;
	if (texture->height * texture->width <= TEXTURE_HEIGHT * TEXTURE_WIDTH)
